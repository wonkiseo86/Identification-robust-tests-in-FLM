...
## The code can produce the simulation results for the model WITHOUT intercept.
...
intrange=3 ; intrange2 = 3 ;  FARmean2=0.6; FARvar2=0.6; minlim=limlim2; maxlim =limlim ; 
...
nnbasis=50; inidiscard=50; kpoint = t
...
## Exogeneity-based benchmark: set Z_t = X_t
...
if(ranfirst==1){coeffs_first=runif(maxbasis,1+limlim2, 1-limlim2)}else{coeffs_first=rep(1,maxbasis)}
  ...
  for (jiter in 2:((T+inidiscard))){
    ...
    x_signal=eta2[,jiter] + operator(LBF[,1:length(dcoefficients1)],dcoefficients1, x_mat[,jiter-1] ,t)
    x_mat[,jiter] = x_signal  + upermean*eta3[1,jiter]   
	gtem1=matrix(0,ncol=1,nrow=nrow(x_mat))
	gtem2=matrix(0,ncol=1,nrow=nrow(x_mat))
	...
	z_mat1[, jiter] <- gtem1 + facz * eta22[, jiter]   #good aux var.
	z_mat2[, jiter] <- gtem2 + facz * eta22[, jiter]   #poor aux var.
	}
...
xscore <- as.numeric(crossprod(devfn_vec * w_int, x_mat[, seq_len(nobs), drop = FALSE]))

YY <- outer(xscore, kappas / sqrt(nobs)) +
...
ldata  <- sweep(Z1n, 2, ydata, "*")
ldata2 <- sweep(Z2n, 2, ydata, "*")
ldataX <- sweep(ZXn, 2, ydata, "*")
...
teststatDX <- statX[paste0("p", PSET[4])]
	...
	TSTATDX[iteration,jjk,tindex]=teststatDX
	   ...
	   if (iteration%%60==0) {print(paste("kernf=",kernf," bw=",bw2," PSET=",PSET[1],PSET[2],PSET[3],PSET[4]," FAR=",FAR," ranbasis=",ranbasis," ranini=",ranini," FARmean=",FARmean1," FARsd=",FARvar1," upermean=",upermean," upersd=",upersd," ranfirst=",ranfirst," limlim=",limlim," limlim2=",limlim2," cviter=",cviter," nointercept",sep=""))}
...
#               "limlim2",limlim2,"cviter",cviter, "facz",facz,"_sizepowerver1.RData",sep=""),
#    TPOWER,TPOWERA,TPOWERB,TPOWERC,TPOWERD,TSTAT,TSTATA,TSTATB,TSTATC,TSTATD,TPOWER2,TPOWERA2,TPOWERB2,TPOWERC2,TPOWERD2,TSTAT2,TSTATA2,TSTATB2,TSTATC2,TSTATD2,TPOWERX,TPOWERAX,TPOWERBX,TPOWERCX,TPOWERDX,TSTATX,TSTATAX,TSTATBX,TSTATCX,TSTATDX,BANDCOL1,BANDCOL2,BANDCOLX)
}
...
############################################################
## How to check entries in the simulation tables
##
## The parameter upermean corresponds to beta_u in the tables.
## The kernel index kernf is coded as follows:
##     kernf = 7: Bartlett,
##     kernf = 1: Parzen.
##
## For any fixed pair of (upermean, kernf), the following lines extract the corresponding table entries from the objects currently in memory.
##
## Table 1: exogeneity-based benchmark size.
## TPOWERX stores the rejection indicators for the benchmark obtained by setting Z_t = X_t. Since kappas = c(20,10,5,0), the 4th column corresponds to H0 and gives empirical size.
##
## To reproduce the beta_u=0, 0.1, and 0.25 columns in Table 1, run the code separately with upermean = 0, 0.1, and 0.25. To reproduce the Bartlett and Parzen columns, run it with kernf = 7 and kernf = 1, respectively.
############################################################

setNames(
  round(100 * colMeans(TPOWERX[, 4, ], na.rm = TRUE), 1),
  paste0("T=", TSET)
)

############################################################
## Table 2: proposed-test rejection rates.
##
